...
function event_spawn(e)
	...
	local cityOwnership = "erudinGuild";
	local cityOwnershipName = "erudinGuildName";
	local cityFactionStatus = "erudinFaction";
	...
	elseif e.self:GetRace() == 78 then
        if e.self:GetTexture() == 0 then
            ...
            if (cityGuild == "") then
                e.self:SetHostile(false);
                e.self:Shout("Setting hostile to false");
		...
		if (cityGuild == tostring(guild_id)) then --if databucket matches guild id
			if char_guild_rank == 2 then --if leader of guild
				...
				elseif(e.message:findi("alert")) then
					e.self:Say("On your orders we can be ["..eq.say_link("hostile").. "] or ["..eq.say_link("peaceful").. "] to all whom enter erudin");
...
function set_base_stats(e)
	...
	elseif (level == "30") then
		local hp = "2000" --+1000 from normal
		local ac = "150"
		local str = "135"
		local sta = "135"

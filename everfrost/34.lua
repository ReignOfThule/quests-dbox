...
function event_signal(e)
    local everfrostGuild = "everfrostGuild";
    ...
    if ( client.valid ) then
        ...
        if (clientGuildName ~= "") then --can't participate in city take overs if guildless
            if (everfrostGuards ~= "") then
                if eq.get_data(everfrostGuildName) ~= tostring(clientGuildName) then --City take over - cannot take over your own city

                    eq.set_data(everfrostGuards, tostring(everfrostGuardsQuantity - 1));
                    local everfrostGuardsFinalQuantity = tonumber(eq.get_data(everfrostGuards));
                    if (everfrostGuardsFinalQuantity > 0) then
                        eq.world_emote(13, "[PvP News] The Everfrost Encampment is under attack!");
                        eq.zone_emote(5, "There are "..everfrostGuardsFinalQuantity.. " remaining mercenaries in the encampment!");

                    elseif (everfrostGuardsFinalQuantity == 0) then
                        if tostring(clientGuildName) == eq.get_data(felwitheGuildName) then --if you own a different city, you wipe out the encampment but don't initiate take over
                            eq.set_data(everfrostGuildName, "Everfrost Encampment"); --set guild name
                            eq.set_data(everfrostGuild, "99"); --set guild ID
                            eq.set_data(everfrostGuards, tostring(maxGuards)); --reset guard count to max
                            eq.world_emote(13, "[PvP News] The Everfrost Encampment has been destroyed by the nobles of <"..clientGuildName..">!");
                        ...
                        elseif tostring(clientGuildName) == eq.get_data(lfayGuildName) then --if you own a different city, you steal guild bank but don't initiate take over
                            ...
                            eq.set_data(everfrostGuildName, tostring(clientGuildName)); --set guild name
                            eq.set_data(everfrostGuild, tostring(clientGuildID)); --set guild ID
                            eq.set_data(everfrostGuards, tostring(maxGuards)); --reset guard count to max
                            eq.stop_all_timers();
                            if eq.get_data(cityRepop) == "no" then
                                eq.set_data(cityRepop, "yes"); --Set databucket to repop 
                            end
                            eq.world_emote(13, "[PvP News] The Everfrost Encampment has been taken over by the nobles of <"..clientGuildName..">!");   
                        end
                    end
                end
            end

...
function event_say(e)
    ...
    if (cityGuild == tostring(guild_id)) then --if databucket matches guild id
        ...
        if(e.message:findi("hail")) then
            e.self:Say("Hail "..char_name.."! Would you like [ "..eq.say_link("STR").." ] , [ "..eq.say_link("STA").."] , [ "..eq.say_link("DEX").."] , [ "..eq.say_link("AGI").."]   ?");
        elseif(e.message:findi("STR")) then
            local player = eq.get_entity_list():GetMob(charID);
            if level == "20" then
    		    player:SpellFinished(147, player); --Spirit Strength
            elseif level == "30" then
    		    player:SpellFinished(151, player); --Raging Strength
            elseif level == "35" then
    		    player:SpellFinished(151, player); --Raging Strength
            elseif level == "40" then
    		    player:SpellFinished(153, player); --Furious Strength
            end
        elseif(e.message:findi("STA")) then
            local player = eq.get_entity_list():GetMob(charID);
            if level == "20" then
    		    player:SpellFinished(279, player); --Spirit of Bear
            elseif level == "30" then
    		    player:SpellFinished(149, player); --Spirit of Ox
            elseif level == "35" then
    		    player:SpellFinished(161, player); --Health
            elseif level == "40" then
    		    player:SpellFinished(161, player); --Health
            end
		elseif(e.message:findi("DEX")) then
            local player = eq.get_entity_list():GetMob(charID);
            if level == "20" then
    		    player:SpellFinished(266, player); --Dexterous Aura
            elseif level == "30" then
    		    player:SpellFinished(349, player); --Rising Dexterity
            elseif level == "35" then
    		    player:SpellFinished(349, player); --Rising Dexterity
            elseif level == "40" then
    		    player:SpellFinished(152, player); --Deftness
            end
		elseif(e.message:findi("AGI")) then
            local player = eq.get_entity_list():GetMob(charID);
            if level == "20" then
    		    player:SpellFinished(148, player); --Spirit of Cat
            elseif level == "30" then
    		    player:SpellFinished(148, player); --Spirit of Cat
            elseif level == "35" then
    		    player:SpellFinished(160, player); --Nimble
            elseif level == "40" then
    		    player:SpellFinished(160, player); --Nimble
            end
		end

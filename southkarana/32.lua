...
function event_signal(e)
    local southkaranaGuild = "southkaranaGuild";
    ...
    if ( client.valid ) then
        ...
        if (clientGuildName ~= "") then --can't participate in city take overs if guildless
            if (southkaranaGuards ~= "") then
                if eq.get_data(southkaranaGuildName) ~= tostring(clientGuildName) then --City take over - cannot take over your own city

                    eq.set_data(southkaranaGuards, tostring(southkaranaGuardsQuantity - 1));
                    local southkaranaGuardsFinalQuantity = tonumber(eq.get_data(southkaranaGuards));
                    if (southkaranaGuardsFinalQuantity > 0) then
                        eq.world_emote(13, "[PvP News] The South Karana Encampment is under attack!");
                        eq.zone_emote(5, "There are "..southkaranaGuardsFinalQuantity.. " remaining mercenaries in the encampment!");

                    elseif (southkaranaGuardsFinalQuantity == 0) then
                        if tostring(clientGuildName) == eq.get_data(felwitheGuildName) then --if you own a different city, you wipe out the encampment but don't initiate take over
                            eq.set_data(southkaranaGuildName, "Faydark Encampment"); --set guild name
                            eq.set_data(southkaranaGuild, "99"); --set guild ID
                            eq.set_data(southkaranaGuards, tostring(maxGuards)); --reset guard count to max
                            eq.stop_all_timers();
                            if eq.get_data(cityRepop) == "no" then
                                eq.set_data(cityRepop, "yes"); --Set databucket to repop 
                            end
                            eq.world_emote(13, "[PvP News] The South Karana Encampment has been destroyed by the nobles of <"..clientGuildName..">!");
                        ...
                        elseif tostring(clientGuildName) == eq.get_data(cauldronGuildName) then --if you own a different city, you steal guild bank but don't initiate take over
                            ...
                            eq.world_emote(13, "[PvP News] The South Karana Encampment has been destroyed by the nobles of <"..clientGuildName..">!");                        
                        elseif tostring(clientGuildName) == eq.get_data(lfayGuildName) then --if you own a different city, you steal guild bank but don't initiate take over
                            eq.set_data(southkaranaGuildName, "Faydark Encampment"); --set guild name
                            eq.set_data(southkaranaGuild, "99"); --set guild ID
                            eq.set_data(southkaranaGuards, tostring(maxGuards)); --reset guard count to max
                            eq.stop_all_timers();
                            ...
                            eq.set_data(southkaranaGuildName, tostring(clientGuildName)); --set guild name
                            eq.set_data(southkaranaGuild, tostring(clientGuildID)); --set guild ID
                            eq.set_data(southkaranaGuards, tostring(maxGuards)); --reset guard count to max
                            eq.stop_all_timers();
                            if eq.get_data(cityRepop) == "no" then
                                eq.set_data(cityRepop, "yes"); --Set databucket to repop 
                            end
                            eq.world_emote(13, "[PvP News] The South Karana Encampment has been taken over by the nobles of <"..clientGuildName..">!");   
                        end
                    end
                end
            end

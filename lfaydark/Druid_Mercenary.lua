...
function event_say(e)
    ...
    if (cityGuild == tostring(guild_id)) then --if databucket matches guild id
        ...
        if(e.message:findi("hail")) then
            e.self:Say("Hail "..char_name.."! Would you like [ "..eq.say_link("sow").." ] , [ "..eq.say_link("thorns").."] , [ "..eq.say_link("FR").."] , [ "..eq.say_link("CR").."] ? I can also [ "..eq.say_link("port").."] you.");
        elseif(e.message:findi("sow")) then
            local player = eq.get_entity_list():GetMob(charID);
            player:SpellFinished(278, player);
        elseif(e.message:findi("thorns")) then
            local player = eq.get_entity_list():GetMob(charID);
            if level == "20" then
    		    player:SpellFinished(273, player); --Shield of Barbs
            elseif level == "30" then
    		    player:SpellFinished(129, player); --Shield of Brambles
            elseif level == "35" then
    		    player:SpellFinished(432, player); --Shield of Spikes
            elseif level == "40" then
    		    player:SpellFinished(432, player); --Shield of Spikes
            end
		elseif(e.message:findi("FR")) then
            local player = eq.get_entity_list():GetMob(charID);
            if level == "20" then
    		    player:SpellFinished(224, player); --Endure Fire
            elseif level == "30" then
    		    player:SpellFinished(60, player); --Resist Fire
            elseif level == "35" then
    		    player:SpellFinished(60, player); --Resist Fire
            elseif level == "40" then
    		    player:SpellFinished(60, player); --Resist Fire
            end
		elseif(e.message:findi("CR")) then
            local player = eq.get_entity_list():GetMob(charID);
            if level == "20" then
    		    player:SpellFinished(225, player); --Endure Cold
            elseif level == "30" then
    		    player:SpellFinished(61, player); --Resist Fire
            elseif level == "35" then
    		    player:SpellFinished(61, player); --Resist Fire
            elseif level == "40" then
    		    player:SpellFinished(61, player); --Resist Fire
		...
		elseif(e.message:findi("port")) then
			 e.self:Say("Would you like to go to [ "..eq.say_link("Butcher").."] ,  [ "..eq.say_link("Commons").."] , [ "..eq.say_link("Feerrott").."] , [ "..eq.say_link("Lavastorm").."] , [ "..eq.say_link("Misty").."] , [ "..eq.say_link("South Ro").."] , [ "..eq.say_link("Steamfont").."] , [ "..eq.say_link("Surefall").."] , or [ "..eq.say_link("Tox").."]?");
		...
		elseif(e.message:findi("Commons")) then
			saved_char_id = charID; --Saving char id for port timer
			local saved_group = e.other:GetGroup(); --Saving group id for port
			saved_group_id = saved_group:GetID();

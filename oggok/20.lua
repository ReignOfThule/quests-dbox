...
function event_signal(e)
    ...
    local oggokBank = "oggokBank";
	local oggokGuardsQuantity = tonumber(eq.get_data(oggokGuards));
    local client = eq.get_entity_list():GetClientByID(e.signal);
	local maxGuards = 23;
    local oggokRepop = "oggokRepop";
    local feerrottRepop = "feerrottRepop";

    local oggokFaction = "oggokFaction";
    ...
    if ( client.valid ) then
        ...
        if (clientGuildName ~= "") then --can't participate in city take overs if guildless
            ...
            if oggokGuardsQuantity > 0 then
                eq.set_data(oggokGuards, tostring(oggokGuardsQuantity - 1));
                local oggokGuardsFinalQuantity = tonumber(eq.get_data(oggokGuards));
                if (oggokGuardsFinalQuantity > 0) then
                    eq.world_emote(13, "[PvP News] The city of Oggok is under attack!");
                    eq.zone_emote(5, "There are "..oggokGuardsFinalQuantity.. " remaining guards in the city!");
                elseif (oggokGuardsFinalQuantity == 0) then
                    local guardsBucketKey = "oggokGuards";
                    local oggokGuards = eq.get_data(guardsBucketKey);
                    local oggokBankAmount = eq.get_data(oggokBank);
                    if eq.get_data(oggokGuildName) ~= tostring(clientGuildName) then --City take over - cannot take over your own city
                        if eq.get_data(qeynosGuildName) == tostring(clientGuildName) then --if you own a different city, you steal guild bank but don't initiate take over
                            client:AddMoneyToPP(0, 0, 0, tonumber(oggokBankAmount), true); --client who got the killing blow receives entire bank amount
                            eq.world_emote(13, "[PvP News] The city of Oggok has been ransacked by the nobles of <"..clientGuildName..">!");
                            eq.world_emote(13, "[PvP News] "..clientName.. " has stolen the Oggok bank!");
                            eq.set_data(oggokGuild, "");
                            eq.set_data(oggokGuildName, "");
                            eq.set_data(oggokFaction, "peaceful");
                            reset_vault_time();
                            if eq.get_data(oggokRepop) == "no" then
                                eq.set_data(oggokRepop, "yes")
                            end
                            if eq.get_data(feerrottRepop) == "no" then
                                eq.set_data(feerrottRepop, "yes")
                            end
                        end
                        ...
                        if eq.get_data(qeynosGuildName) ~= tostring(clientGuildName) then --If guild doesn't own Qeynos
                            if eq.get_data(felwitheGuildName) ~= tostring(clientGuildName) then --If guild doesn't own Felwithe
                                if eq.get_data(erudinGuildName) ~= tostring(clientGuildName) then --If guild doesn't own Erudin
                                    eq.set_data(oggokGuildName, tostring(clientGuildName)); --set guild name
                                    eq.set_data(oggokGuild, tostring(clientGuildID)); --set guild ID
                                    ...
                                    eq.world_emote(13, "[PvP News] The city of Oggok has been taken over by the nobles of <"..clientGuildName..">!");
                                    eq.world_emote(13, "[PvP News] "..clientName.. " has stolen the Oggok bank!");
                                end
                            end
                        end
